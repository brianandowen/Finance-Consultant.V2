...
exports.analyzeFinanceQnA = analyzeFinanceQnA;
...
async function analyzeFinanceSummary(data) {
    const sys = "你是理財分析助理。只能依據我提供的統計摘要(JSON)回答；" +
        "輸出：1) 本月重點(<=3)、2) 風險/異常(<=2)、3) 下週行動建議(3)。語氣精煉，不重述 JSON，不要求更多個資。";
    const user = "以下是財務摘要 JSON（不要重述原文）：\n" +
    ...
    // 印出 usage 方便對照後台
    // @ts-ignore
    console.log("[LLM] id:", res.id, "usage:", res.usage || res.meta?.usage);
    // @ts-ignore
    return String(res.output_text ?? "").trim();
}
/** 依據「摘要 JSON + 使用者問題」產生個人化回答（給 /ask 與專用頻道） */
async function analyzeFinanceQnA(data, question) {
    const sys = [
    ...
    // @ts-ignore
    console.log("[LLM-QnA] id:", res.id, "usage:", res.usage || res.meta?.usage);
    // @ts-ignore
    return String(res.output_text ?? "").trim();
}
