...
import { FinanceSummary } from "../lib/llm";

export async function buildFinanceSummary(db: Pool, userId: string): Promise<FinanceSummary> {
  // 以台北時區計算當月區間（[含, 不含)）
  ...
  const byCategoryTop3 = catRows.rows.map((r: any) => ({
    name: r.name ?? "未分類",
  ...
  let goal:
    | { name: string; target: number; saved: number; progressPct: number }
    | undefined;
  ...
  if (goalRow.rowCount) {
    ...
    const startDate = g.start_date
      ? DateTime.fromJSDate(new Date(g.start_date)).toUTC().toISO()
  ...
  if (eat && eat.pct >= 15) alerts.push(`外食占比 ${eat.pct}%（>15%）`);
